def draw_wire(instructions)
  ...
  instructions.each do |instruction|
    ...
    distance = instruction[1,instruction.size-1].to_i

    case direction
    when 'R' then distance.times { pointer = [pointer[0]+1,pointer[1]]; wire << pointer }
    when 'L' then distance.times { pointer = [pointer[0]-1,pointer[1]]; wire << pointer }
    when 'U' then distance.times { pointer = [pointer[0],pointer[1]+1]; wire << pointer }
    when 'D' then distance.times { pointer = [pointer[0],pointer[1]-1]; wire << pointer }
    end
  end
...
def get_distance(instructions, destination)
  ...
  instructions.each do |instruction|
    ...
    case direction
    when 'R' then distance.times { pointer = [pointer[0]+1,pointer[1]]; step+=1; return step if pointer == destination}
    when 'L' then distance.times { pointer = [pointer[0]-1,pointer[1]]; step+=1; return step if pointer == destination }
    when 'U' then distance.times { pointer = [pointer[0],pointer[1]+1]; step+=1; return step if pointer == destination }
    when 'D' then distance.times { pointer = [pointer[0],pointer[1]-1]; step+=1; return step if pointer == destination }
    end
  end
end

closest = intersections.map do |intersection|
  get_distance(instructions1, intersection) + get_distance(instructions2, intersection)
end.min
